const state={
  ...
  token:'',publicConfig:null,handover:[],handoverArea:'',handoverQuery:'',authStatus:null,
  bundleCache:{},offline:false,lastSyncAt:null,editorDraftKey:'',editorSaveTimer:null,editorDirty:false,
  currentAdmissionId:'',localDbReady:false,uiMode:'mobile',searchTimer:null,patientNoteLimit:24
};
...
function detectUiMode_(){
  const coarse=!!(window.matchMedia&&matchMedia('(pointer:coarse)').matches);
  const noHover=!!(window.matchMedia&&matchMedia('(hover:none)').matches);
  const fineDesktop=!!(window.matchMedia&&matchMedia('(pointer:fine)').matches&&matchMedia('(hover:hover)').matches&&window.innerWidth>=760);
  state.uiMode=fineDesktop&&!coarse&&!noHover?'desktop':'mobile';
  document.documentElement.dataset.ui=state.uiMode;
  document.documentElement.classList.toggle('touch-ui',state.uiMode==='mobile');
  document.documentElement.classList.toggle('desktop-ui',state.uiMode==='desktop');
}
function leaveEditorMode_(){document.body.classList.remove('editor-mode');}
function enterEditorMode_(){document.body.classList.add('editor-mode');}
function isMobileUi_(){return state.uiMode!=='desktop';}
detectUiMode_();
let uiResizeTimer_=null;
window.addEventListener('resize',()=>{clearTimeout(uiResizeTimer_);uiResizeTimer_=setTimeout(detectUiMode_,120)},{passive:true});
window.addEventListener('orientationchange',()=>setTimeout(detectUiMode_,120),{passive:true});
...
function showFatalStartup_(err){
  ...
  root.innerHTML=`<div class="auth-card"><h1>ICU Clinical Notes</h1><div class="danger-note"><b>Startup error</b><br><br>${esc(msg)}</div><div class="auth-actions"><button class="btn" onclick="location.reload()">Retry</button></div><div class="auth-note">Frontend v8 · GitHub SPA assets + Apps Script database RPC.</div></div>`;
}
...
function bootApp_(){
  try{
    renderLogin();
    state.token=safeStorageGet_('icu_notes_session_v8') || safeStorageGet_('icu_notes_session_v7') || safeStorageGet_('icu_notes_session_v6');
    ...
    publicCall('getPublicConfig',null,cfg=>{
      ...
      publicCall('getAuthStatus',state.token,st=>{
        if(st&&st.authenticated){state.authStatus=st;startApp(st.user);} 
...
function clearSession(){state.token='';safeStorageRemove_('icu_notes_session_v8');safeStorageRemove_('icu_notes_session_v7');safeStorageRemove_('icu_notes_session_v6');}
function setSyncBanner_(message,kind=''){
  const el=document.getElementById('syncBanner');if(!el)return;
  if(!message){el.className='sync-banner hidden';el.textContent='';return;}
  el.textContent=message;el.className='sync-banner'+(kind?' '+kind:'');
}
function applyBootstrap_(boot,fromCache=false){
  if(!boot)return;
  state.boot=boot;state.active=boot.activeAdmissions||[];state.lastSyncAt=fromCache?state.lastSyncAt:new Date().toISOString();
  document.getElementById('brand').textContent=boot.appName||'ICU Clinical Notes';
  const u=boot.currentUser||state.authStatus?.user||{};
  document.getElementById('userId').textContent=(u.displayName||boot.userId||'')+(u.designation?' · '+u.designation:'');
  renderAreaNav();
  if(state.view==='handover')renderHandoverView();else if(state.view==='patient'&&state.bundle)renderPatient();else renderDashboard();
  if(fromCache)setSyncBanner_('Cached census loaded · refreshing from Google Sheets…','offline');
  else setSyncBanner_('Synced '+new Date().toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'}));
}
async function startApp(user){
  document.getElementById('authRoot').classList.add('hidden');
  document.getElementById('appRoot').classList.remove('hidden');
  const uid=(user&&user.userId)||state.authStatus?.user?.userId||state.authStatus?.user?.username||'current';
  let cached=null;
  try{if(window.ICULocalDB){await ICULocalDB.open();state.localDbReady=true;cached=await ICULocalDB.getBootstrap(uid);if(cached)applyBootstrap_(cached,true);}}catch(e){console.warn('Local DB unavailable',e);}
  run('getBootstrap',null,async boot=>{
    state.offline=false;applyBootstrap_(boot,false);
    try{if(window.ICULocalDB)await ICULocalDB.cacheBootstrap(boot.userId||uid,boot);}catch(e){console.warn(e)}
    updateLocalDraftCount_();
  },err=>{
    state.offline=true;
    if(cached){applyBootstrap_(cached,true);setSyncBanner_('Offline / backend unavailable · cached data shown. Drafting is allowed locally; signing is disabled.','offline');updateLocalDraftCount_();}
    else{setSyncBanner_('Unable to load clinical database: '+err.message,'error');document.getElementById('main').innerHTML='<div class="empty">Clinical database unavailable. Check the connection and use Refresh when online.</div>';}
  });
}
...
function renderLogin(message=''){
  ...
  root.innerHTML=`<div class="auth-card">
    ...
    <div class="auth-note">Registration is only accepted if your email is already listed by the administrator in <b>TRUSTED_EMAILS</b>.</div><div class="auth-note" id="loginConnectionStatus">Frontend v8 loaded · connecting database…</div>
  </div>`;
}

function submitLogin(){
  publicCall('loginUser',{username:val('lgUser'),password:val('lgPass')},r=>{
    state.token=r.token;safeStorageSet_('icu_notes_session_v8',r.token);startApp();
  });
}
...
async function logout(){
  const uid=currentUserId_();let count=0;try{if(window.ICULocalDB)count=(await ICULocalDB.listDrafts(uid)).length}catch(e){}
  if(count&&!confirm(`There are ${count} local unsynced draft(s) on this device. Logging out will remove local clinical caches and drafts. Continue?`))return;
  const tok=state.token;try{if(window.ICULocalDB)await ICULocalDB.clearUser(uid)}catch(e){}clearSession();publicCall('logoutUser',tok,()=>renderLogin('You have logged out.'),()=>renderLogin('You have logged out.'));
}
...
function currentUserId_(){return state.boot?.userId||state.authStatus?.user?.userId||state.authStatus?.user?.username||'current'}
function isOnline_(){return navigator.onLine!==false&&!state.offline}
async function refreshData(showToast=true){
  if(!state.token)return;
  setSyncBanner_('Refreshing active census from Google Sheets…');
  run('getBootstrap',null,async b=>{state.offline=false;applyBootstrap_(b,false);try{if(window.ICULocalDB)await ICULocalDB.cacheBootstrap(currentUserId_(),b)}catch(e){}if(showToast)toast('Clinical data refreshed.');updateLocalDraftCount_();},e=>{state.offline=true;setSyncBanner_('Refresh failed · cached data remains available. '+e.message,'offline');});
}
window.addEventListener('online',()=>{state.offline=false;setSyncBanner_('Connection restored · refresh recommended.');});
window.addEventListener('offline',()=>{state.offline=true;setSyncBanner_('Offline · unsaved note text continues to save locally. Signing is disabled.','offline');});
...
function refreshDashboard(cb){refreshData(false);if(cb)cb()}
function goDashboard(){leaveEditorMode_();state.view='dashboard';state.bundle=null;state.currentNote=null;renderDashboard()}


function goHandover(area){
  leaveEditorMode_();state.view='handover';
...
function renderHandoverView(){
  ...
  document.getElementById('main').innerHTML=`
    ...
    <div class="section-title"><h2>Handover</h2><span class="count">Latest signed clinical entry</span></div>
    <div id="handoverBoard"><div class="empty">Loading handover…</div></div>`;
  state.handover=buildLocalHandoverRows_(area,state.handoverQuery||'');renderHandoverRows();
}
function buildLocalHandoverRows_(area,query){
  const q=String(query||'').trim().toLowerCase();
  return (state.active||[]).filter(x=>(!area||area==='ALL'||x.admission.Area===area)).filter(x=>!q||[x.patient.Name,x.patient.RN,x.admission.Ward_Name,x.admission.Bed].some(v=>String(v||'').toLowerCase().includes(q))).map(x=>{
    const latest=x.latestEntry||null;const text=latest?(latest.Simple_Entry||([latest.Assessment,latest.Plan].filter(Boolean).join('\nPlan: '))||latest.Problems||latest.HOPI||latest.Procedure_Name||''):'';
    return {admissionId:x.admission.Admission_ID,patientId:x.patient.Patient_ID,name:x.patient.Name,rn:x.patient.RN,age:x.patient.Age,sex:x.patient.Sex,comorbidities:x.patient.Comorbidities,area:x.admission.Area,wardName:x.admission.Ward_Name||'',bed:x.admission.Bed||'',wardBed:x.admission.Ward_Bed||'',diagnosis:x.admission.Primary_Diagnosis,shortHopi:x.admission.Short_Presenting_History,problems:x.activeProblemSummary||(x.latestDaily&&x.latestDaily.Problems)||'',icuDay:x.icuDay,latestEntry:latest?{noteId:latest.Note_ID,type:latest.Entry_Type,dateTime:latest.Note_DateTime,authorName:latest.Author_Name||latest.Author_UserID||'',text:text.slice(0,1200)}:null};
  });
}
...
function renderDashboard(){
  leaveEditorMode_();state.view='dashboard';
  ...
  document.getElementById('main').innerHTML=`
    <div class="toolbar">
      ...
      <button class="btn mobile-hide" onclick="goHandover()">Handover</button>
      <button class="btn" onclick="openLocalDrafts()"><span id="localDraftCount">Local drafts</span></button>
      <button class="btn" onclick="refreshData()">↻ Refresh</button>
      <button class="btn mobile-hide" onclick="openSummaryCards()">Summary cards</button><button class="btn mobile-hide" onclick="openAreaHandover()">Print handover</button>
      <span class="spacer"></span>
      <input class="search" id="searchInput" placeholder="Search active list by name or RN" oninput="scheduleDashboardFilter_()">
      <button class="btn mobile-hide" onclick="doSearch()">Search all records</button>
    </div>
    ...
    <div class="cache-note">Navigation uses the locally cached active census. Use Refresh when you need an immediate server re-read.</div><div class="grid" id="dashboardGrid">${list.length?list.map(patientCard).join(''):'<div class="empty" style="grid-column:1/-1">No active patients in this area.</div>'}</div>`;setTimeout(updateLocalDraftCount_,0);
}
...
async function openPatient(admissionId){
  state.currentAdmissionId=admissionId;state.view='patient';state.patientNoteLimit=isMobileUi_()?24:60;let cached=state.bundleCache[admissionId]||null;
  if(!cached&&window.ICULocalDB){try{cached=await ICULocalDB.getBundle(currentUserId_(),admissionId)}catch(e){}}
  if(cached){state.bundle=cached;renderPatient();setSyncBanner_('Cached patient record shown · refreshing…','offline');
    run('getPatientBundle',admissionId,async b=>{state.offline=false;state.bundleCache[admissionId]=b;if(state.currentAdmissionId===admissionId&&state.view!=='editor'){state.bundle=b;renderPatient();setSyncBanner_('Patient record refreshed '+new Date().toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'}));}try{if(window.ICULocalDB)await ICULocalDB.cacheBundle(currentUserId_(),admissionId,b)}catch(e){}},e=>{state.offline=true;setSyncBanner_('Offline / refresh failed · cached patient record shown. Draft locally; do not sign until reconnected.','offline');});
    return cached;
  }
  return await new Promise(resolve=>run('getPatientBundle',admissionId,async b=>{state.offline=false;state.bundle=b;state.bundleCache[admissionId]=b;renderPatient();setSyncBanner_('Patient record loaded');try{if(window.ICULocalDB)await ICULocalDB.cacheBundle(currentUserId_(),admissionId,b)}catch(e){}resolve(b);},e=>{state.offline=true;document.getElementById('main').innerHTML='<div class="empty">Unable to load patient record.</div>';resolve(null);}));
}
function renderPatient(){
  leaveEditorMode_();state.view='patient';state.currentEntryType='';const {patient:p,admission:a,notes}=state.bundle;
  const limit=Math.max(1,state.patientNoteLimit||24),shown=(notes||[]).slice(0,limit),more=(notes||[]).length-shown.length;
  document.getElementById('main').innerHTML=`
    ${state.offline?'<div class="offline-lock"><b>Offline / cached view.</b> Confirm current clinical data after reconnecting. You can continue drafting locally, but signing is disabled.</div>':''}
    ...
    <div class="toolbar"><button class="btn primary" onclick="newNote('${a.Admission_ID}','Daily Review')">+ Daily Review</button><button class="btn" onclick="openEntryMenu()">Other entry</button><button class="btn" onclick="goDashboard()">← Patients</button></div>
    ...
    <div>${shown.length?shown.map(noteRow).join(''):'<div class="empty">No clinical entries yet.</div>'}${more>0?`<button class="btn" style="width:100%;margin:9px 0" onclick="showMorePatientNotes_()">Show ${Math.min(more,isMobileUi_()?24:60)} older entries · ${more} remaining</button>`:''}</div>`;
  window.scrollTo({top:0,behavior:'auto'});
}
function showMorePatientNotes_(){state.patientNoteLimit+=(isMobileUi_()?24:60);renderPatient();}
...
function refreshPatientBundle(cb){const id=state.bundle.admission.Admission_ID;run('getPatientBundle',id,async b=>{state.bundle=b;state.bundleCache[id]=b;try{if(window.ICULocalDB)await ICULocalDB.cacheBundle(currentUserId_(),id,b)}catch(e){}if(cb)cb();else renderPatient()})}
...
function closeOcr(){
  ...
  if(isMobileUi_()&&state.ocrWorker){try{state.ocrWorker.terminate()}catch(_){}state.ocrWorker=null;}
...
async function openOcrAssist(context,preferredTarget=''){
  ...
  document.getElementById('ocrHost').innerHTML=`<div class="modal-back" style="z-index:80" onclick="if(event.target===this)closeOcr()"><div class="modal wide">
    ...
    <div class="modal-body">
      <div class="form-grid">
        <div class="field"><label>Insert / detect into</label><select id="ocrTarget">${candidates.map(x=>`<option value="${esc(x[0])}" ${preferredTarget===x[0]?'selected':''}>${esc(x[1])}</option>`).join('')}</select></div>
...
async function loadOcrImage(file,context){
  ...
  try{
    ...
    const mobile=isMobileUi_();state.ocrOriginalCanvas=await fileToCanvas(file,mobile?1400:2300);
...
function preprocessCanvas(source,contrastLevel,sharpen){
  const mobile=window.matchMedia&&window.matchMedia('(max-width:700px)').matches;const maxW=mobile?1350:1850,maxH=mobile?1850:2500,scale=Math.min(1,maxW/source.width,maxH/source.height);const w=Math.max(1,Math.round(source.width*scale)),h=Math.max(1,Math.round(source.height*scale));
...
async function newNote(admissionId,type,seed=null){if(!state.bundle||state.bundle.admission.Admission_ID!==admissionId){const b=await openPatient(admissionId);if(b)openNoteEditor(type,seed)}else openNoteEditor(type,seed)}
function editNote(noteId){const n=state.bundle.notes.find(x=>x.Note_ID===noteId);if(n)openNoteEditor(n.Entry_Type,n)}

function draftKeyFor_(type,seed){const noteId=seed?.Note_ID||'';return [currentUserId_(),state.bundle.admission.Admission_ID,type,noteId||'NEW'].join('::')}
function payloadToSeed_(p){return {Note_ID:p.noteId||'',Note_DateTime:p.noteDateTime,Entry_Type:p.entryType,Status:p.noteId?'DRAFT':'LOCAL',Copied_From_Note_ID:p.copiedFromNoteId||'',HOPI:p.hopi||'',Problems:p.problems||'',CNS:p.cns||'',CVS:p.cvs||'',Respiratory:p.respiratory||'',GIT:p.git||'',Renal:p.renal||'',Haematology:p.haematology||'',Infection:p.infection||'',Indwelling_Catheters:p.indwellingCatheters||'',Other_Investigations:p.otherInvestigations||'',Assessment:p.assessment||'',Plan:p.plan||'',Simple_Title:p.simpleTitle||'',Simple_Entry:p.simpleEntry||'',Procedure_Name:p.procedureName||''}}
async function openNoteEditor(type,seed=null){
  let actualSeed=seed||null;const key=draftKeyFor_(type,seed);
  try{if(window.ICULocalDB){const local=await ICULocalDB.getDraft(key);if(local&&local.payload){const serverTime=seed?.Updated_At?new Date(seed.Updated_At).getTime():0;const localTime=new Date(local.updatedAt).getTime();if(!seed||localTime>serverTime){if(confirm('A newer unsaved local version exists from '+new Date(local.updatedAt).toLocaleString('en-MY')+'. Restore it?'))actualSeed=payloadToSeed_(local.payload);}}}}catch(e){console.warn(e)}
  state.currentNote=actualSeed||null;state.currentEntryType=type;state.view='editor';state.editorDraftKey=key;state.editorDirty=false;enterEditorMode_();
  const p=state.bundle.patient,a=state.bundle.admission,n=actualSeed||{};const daily=type==='Daily Review';
  const location=[a.Area,a.Ward_Bed].filter(Boolean).join(' · ');
  document.getElementById('main').innerHTML=`
    ${state.offline?'<div class="offline-lock"><b>Offline drafting mode.</b> Every change is saved on this device. Reconnect before signing.</div>':''}
    <div class="editor-shell" id="noteEditorForm">
      <div class="editor-toolbar">
        <button class="btn" onclick="flushLocalDraft_();renderPatient()">← Patient</button>
        ${daily&&!actualSeed?'<button class="btn" onclick="copyPreviousDaily()">Copy previous</button>':''}
        <button class="btn" onclick="openOcrAssist('NOTE')">📷 OCR</button>
        ${daily?'<button class="btn" onclick="toggleReviewSections_(true)">Expand all</button><button class="btn" onclick="toggleReviewSections_(false)">Collapse</button>':''}
      </div>
      <div class="review-header">
        <div class="review-patient">${esc(type)} · ${esc(p.Name)}</div>
        <div class="review-meta">RN ${esc(p.RN)} · ${esc(location)}${daily?' · <span id="reviewProgress">0/'+DAILY_FIELDS.length+' sections</span>':''}</div>
      </div>
      <details class="editor-meta"><summary>Entry details · date/time & author</summary>
        <div class="form-grid"><div class="field"><label>Date / time</label><input id="noteDateTime" type="datetime-local" value="${n.Note_DateTime?toLocalInput(n.Note_DateTime):todayLocal()}"></div><div class="field"><label>Status / author</label><div style="padding:9px">${n.Status?esc(n.Status):'NEW ENTRY'} · ${esc(state.boot.currentUser?.displayName||state.boot.userId)}${state.boot.currentUser?.designation?' · '+esc(state.boot.currentUser.designation):''}</div></div></div>
      </details>
      ${daily?dailyForm(n):simpleForm(type,n)}
      <div class="sticky-actions"><button class="btn" onclick="saveCurrent(false)">Save draft</button><button class="btn primary" ${state.offline?'disabled':''} onclick="saveCurrent(true)">Sign & save</button><div class="local-save-status" id="localSaveStatus">Local autosave active</div></div>
    </div>`;
  activateEditorAutosave_();if(daily)activateReviewEditor_();
  window.scrollTo({top:0,behavior:'auto'});
}
function dailyForm(n){
  const chips=[['hopi','HOPI',true],['problems','Problems',true],['cns','CNS'],['cvs','CVS'],['respiratory','Resp'],['git','GIT'],['renal','Renal'],['haematology','Haem'],['infection','Infection'],['indwellingCatheters','Lines'],['otherInvestigations','Ix'],['assessment','Assessment',true],['plan','Plan',true]];
  return `<div class="review-jump-wrap"><div class="review-jump">${chips.map(x=>`<button type="button" class="review-chip ${x[2]?'core':''}" onclick="scrollReviewSection_('${x[0]}')">${x[1]}</button>`).join('')}</div></div>
  <div class="review-sections">
    <div class="review-group-title">Clinical summary</div>
    ${reviewSection_('hopi','HOPI / Clinical Summary',n.HOPI,true)}
    ${reviewSection_('problems','Problems / Issues',n.Problems,true)}
    <div class="review-group-title">Review by system</div>
    ${reviewSection_('cns','CNS',n.CNS)}
    ${reviewSection_('cvs','CVS',n.CVS)}
    ${reviewSection_('respiratory','Respiratory',n.Respiratory)}
    ${reviewSection_('git','GIT / Nutrition',n.GIT)}
    ${reviewSection_('renal','Renal / Fluid',n.Renal)}
    ${reviewSection_('haematology','Haematology / Blood Investigations',n.Haematology)}
    ${reviewSection_('infection','Infection / Infective Parameters',n.Infection)}
    ${reviewSection_('indwellingCatheters','Indwelling Catheters / Lines',n.Indwelling_Catheters)}
    ${reviewSection_('otherInvestigations','Other Investigations',n.Other_Investigations)}
    <div class="review-group-title">Assessment and plan</div>
    ${reviewSection_('assessment','Assessment',n.Assessment)}
    ${reviewSection_('plan','Plan',n.Plan)}
  </div>`;
}
function reviewSection_(id,label,value,open=false){
  const has=String(value||'').trim().length>0;
  return `<details class="review-section ${has?'has-text':''}" id="section-${id}" ${open?'open':''}><summary>${esc(label)}</summary><div class="review-body"><textarea id="${id}" rows="4" placeholder="Tap to enter ${esc(label.toLowerCase())}…">${esc(value||'')}</textarea><div class="review-field-actions"><button type="button" class="btn" onclick="openOcrAssist('NOTE','${id}')">📷 OCR</button><button type="button" class="btn" onclick="nextReviewSection_('${id}')">Next →</button></div></div></details>`;
}
function simpleForm(type,n){return `<div class="review-sections" style="padding-top:10px">${type==='Procedure'?`<div class="field"><label>Procedure</label><input id="procedureName" value="${esc(n.Procedure_Name||'')}"></div>`:''}<div class="field" style="margin-top:9px"><label>${type==='Family Discussion'?'Family / persons present / reason':'Title / reason'}</label><input id="simpleTitle" value="${esc(n.Simple_Title||'')}"></div><div class="field" style="margin-top:9px"><label>${esc(type)} entry</label><textarea class="tall" id="simpleEntry" placeholder="Enter clinical note…">${esc(n.Simple_Entry||'')}</textarea></div></div>`}
...
function scrollReviewSection_(id){const d=document.getElementById('section-'+id);if(!d)return;d.open=true;requestAnimationFrame(()=>{d.scrollIntoView({behavior:'smooth',block:'start'});setTimeout(()=>document.getElementById(id)?.focus({preventScroll:true}),260)});}
function toggleReviewSections_(open){document.querySelectorAll('.review-section').forEach(x=>x.open=!!open);}
function nextReviewSection_(id){const i=DAILY_FIELDS.indexOf(id),next=DAILY_FIELDS[i+1];if(next)scrollReviewSection_(next);else{document.querySelector('.sticky-actions')?.scrollIntoView({behavior:'smooth',block:'end'});toast('Daily Review sections complete. Review before signing.');}}
function updateReviewProgress_(){let filled=0;DAILY_FIELDS.forEach(id=>{const el=document.getElementById(id),sec=document.getElementById('section-'+id),has=!!String(el?.value||'').trim();if(has)filled++;if(sec)sec.classList.toggle('has-text',has)});const p=document.getElementById('reviewProgress');if(p)p.textContent=filled+'/'+DAILY_FIELDS.length+' sections';}
function activateReviewEditor_(){updateReviewProgress_();const host=document.querySelector('.review-sections');if(!host)return;host.addEventListener('input',e=>{if(e.target&&e.target.tagName==='TEXTAREA')updateReviewProgress_()},{passive:true});}
...
function currentEditorPayload_(){const type=state.currentEntryType||state.currentNote?.Entry_Type||'Review';return {noteId:state.currentNote?.Note_ID||'',admissionId:state.bundle.admission.Admission_ID,noteDateTime:val('noteDateTime'),entryType:type,sign:false,copiedFromNoteId:state.currentNote?.Copied_From_Note_ID||state.currentNote?.copiedFromNoteId||'',hopi:val('hopi'),problems:val('problems'),cns:val('cns'),cvs:val('cvs'),respiratory:val('respiratory'),git:val('git'),renal:val('renal'),haematology:val('haematology'),infection:val('infection'),indwellingCatheters:val('indwellingCatheters'),otherInvestigations:val('otherInvestigations'),assessment:val('assessment'),plan:val('plan'),simpleTitle:val('simpleTitle'),simpleEntry:val('simpleEntry'),procedureName:val('procedureName')}}
function setLocalSaveStatus_(text,kind=''){const el=document.getElementById('localSaveStatus');if(el){el.textContent=text;el.className='local-save-status'+(kind?' '+kind:'')}}
function activateEditorAutosave_(){const host=document.getElementById('noteEditorForm');if(!host)return;host.addEventListener('input',()=>{state.editorDirty=true;setLocalSaveStatus_('Saving locally…');clearTimeout(state.editorSaveTimer);state.editorSaveTimer=setTimeout(flushLocalDraft_,700)});flushLocalDraft_();}
async function flushLocalDraft_(){if(!state.editorDraftKey||state.view!=='editor'||!state.bundle||!window.ICULocalDB)return;try{const payload=currentEditorPayload_();await ICULocalDB.putDraft({key:state.editorDraftKey,userId:currentUserId_(),admissionId:payload.admissionId,noteId:payload.noteId||'',entryType:payload.entryType,patientName:state.bundle.patient.Name,rn:state.bundle.patient.RN,payload});state.editorDirty=false;setLocalSaveStatus_('Saved locally · '+new Date().toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'}),'ok');updateLocalDraftCount_();}catch(e){setLocalSaveStatus_('Local autosave failed: '+e.message,'warn')}}
async function saveCurrent(sign){
  await flushLocalDraft_();const p=currentEditorPayload_();p.sign=!!sign;
  if(state.offline||navigator.onLine===false){toast(sign?'Cannot sign while offline. Your work is saved locally.':'Saved locally. Reconnect to save the draft to Google Sheets.',sign);return;}
  setLocalSaveStatus_('Saving to Google Sheets…');
  run('saveNote',p,async n=>{try{if(window.ICULocalDB&&state.editorDraftKey)await ICULocalDB.deleteDraft(state.editorDraftKey)}catch(e){}state.editorDraftKey='';toast(sign?'Note signed.':'Draft saved to Google Sheets.');const id=state.bundle.admission.Admission_ID;run('getPatientBundle',id,async b=>{state.bundle=b;state.bundleCache[id]=b;try{if(window.ICULocalDB)await ICULocalDB.cacheBundle(currentUserId_(),id,b)}catch(e){}renderPatient();refreshData(false);updateLocalDraftCount_();})},e=>{state.offline=true;setLocalSaveStatus_('Server save failed · local copy retained','warn');toast('Server save failed. Your work remains saved locally.',true)});
}
...
function applyCopy(n){document.querySelectorAll('[data-copy]:checked').forEach(ch=>{const id=ch.dataset.copy,key={hopi:'HOPI',problems:'Problems',cns:'CNS',cvs:'CVS',respiratory:'Respiratory',git:'GIT',renal:'Renal',haematology:'Haematology',infection:'Infection',indwellingCatheters:'Indwelling_Catheters',otherInvestigations:'Other_Investigations',assessment:'Assessment',plan:'Plan'}[id];const el=document.getElementById(id);if(el)el.value=n[key]||''});state.currentNote=state.currentNote||{};state.currentNote.copiedFromNoteId=n.Note_ID;closeModal();updateReviewProgress_();flushLocalDraft_();toast('Selected sections copied. Review and update before signing.')}

function scheduleDashboardFilter_(){clearTimeout(state.searchTimer);state.searchTimer=setTimeout(filterDashboardLocal_,120)}
function filterDashboardLocal_(){const q=String(val('searchInput')||'').trim().toLowerCase();const list=(state.active||[]).filter(x=>(state.area==='ALL'||x.admission.Area===state.area)&&(!q||String(x.patient.Name||'').toLowerCase().includes(q)||String(x.patient.RN||'').toLowerCase().includes(q)));const host=document.getElementById('dashboardGrid');if(host)host.innerHTML=list.length?list.map(patientCard).join(''):'<div class="empty" style="grid-column:1/-1">No matching active patient.</div>'}
async function updateLocalDraftCount_(){const el=document.getElementById('localDraftCount');if(!window.ICULocalDB)return;try{const rows=await ICULocalDB.listDrafts(currentUserId_());if(el)el.textContent=rows.length?`Local drafts (${rows.length})`:'Local drafts';}catch(e){}}
async function openLocalDrafts(){if(!window.ICULocalDB)return toast('Local database unavailable.',true);const rows=await ICULocalDB.listDrafts(currentUserId_());modal('Unsynced / local drafts',rows.length?rows.map(d=>`<div class="note-row"><div><span class="badge local">LOCAL</span><div class="meta">${fmtDate(d.updatedAt)}</div></div><div><b>${esc(d.patientName||'Patient')}</b><div class="meta">RN ${esc(d.rn||'')} · ${esc(d.entryType||'')}</div></div><div><button class="btn small primary" onclick="closeModal();resumeLocalDraft('${esc(d.key)}')">Resume</button><button class="btn small danger" onclick="deleteLocalDraft('${esc(d.key)}')">Delete</button></div></div>`).join(''):'<div class="empty">No local drafts.</div>','',true)}
async function resumeLocalDraft(key){const d=await ICULocalDB.getDraft(key);if(!d)return toast('Local draft no longer exists.',true);const b=await openPatient(d.admissionId);if(b)openNoteEditor(d.entryType,payloadToSeed_(d.payload))}
async function deleteLocalDraft(key){if(!confirm('Delete this local draft from this device?'))return;await ICULocalDB.deleteDraft(key);closeModal();updateLocalDraftCount_();toast('Local draft deleted.')}
...
// v8 resilience: flush unsaved editor content when app is backgrounded or closed.
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')flushLocalDraft_();});
window.addEventListener('pagehide',()=>{flushLocalDraft_();});
window.addEventListener('beforeunload',()=>{flushLocalDraft_();});
