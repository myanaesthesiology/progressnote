const state={
  ...
  currentAdmissionId:'',localDbReady:false
};
...
function goDashboard(){state.view='dashboard';state.bundle=null;state.currentNote=null;renderDashboard()}
...
function renderDashboard(){
  ...
  document.getElementById('main').innerHTML=`
    <div class="toolbar">
      ...
      <button class="btn" onclick="goHandover()">Handover</button>
      ...
      <span class="spacer"></span>
      <input class="search" id="searchInput" placeholder="Search active list by name or RN" oninput="filterDashboardLocal_()">
...
async function openPatient(admissionId){
  state.currentAdmissionId=admissionId;state.view='patient';let cached=state.bundleCache[admissionId]||null;
...
async function loadOcrImage(file,context){
  ...
  try{
    ...
    const mobile=window.matchMedia&&window.matchMedia('(max-width:700px)').matches;state.ocrOriginalCanvas=await fileToCanvas(file,mobile?1550:2200);
...
async function openNoteEditor(type,seed=null){
  ...
  state.currentNote=actualSeed||null;state.view='editor';state.editorDraftKey=key;state.editorDirty=false;
  const p=state.bundle.patient,a=state.bundle.admission,n=actualSeed||{};const daily=type==='Daily Review';
  document.getElementById('main').innerHTML=`
    ${state.offline?'<div class="offline-lock"><b>Offline drafting mode.</b> Every change is saved in this device. Reconnect before signing or saving to Google Sheets.</div>':''}
    <div class="toolbar"><button class="btn" onclick="flushLocalDraft_();renderPatient()">← Patient timeline</button>${daily&&!seed?'<button class="btn" onclick="copyPreviousDaily()">Copy previous</button>':''}<button class="btn" onclick="openOcrAssist('NOTE')">📷 OCR</button></div>
    <div class="form" id="noteEditorForm"><div class="review-header">${esc(type)} · ${esc(p.Name)} · RN ${esc(p.RN)}</div>
  ...
  activateEditorAutosave_();
}
function dailyForm(n){return `<h3 class="system-title">Clinical summary</h3><div class="form-grid">${ta('hopi','HOPI / Clinical Summary',n.HOPI,true)}${ta('problems','Problems / Issues',n.Problems,true)}</div><h3 class="system-title">Review by system</h3><div class="form-grid">${ta('cns','CNS',n.CNS)}${ta('cvs','CVS',n.CVS)}${ta('respiratory','Respiratory',n.Respiratory)}${ta('git','GIT / Nutrition',n.GIT)}${ta('renal','Renal / Fluid',n.Renal)}${ta('haematology','Haematology / Blood Ix',n.Haematology)}${ta('infection','Infection / Infective Parameters',n.Infection)}${ta('indwellingCatheters','Indwelling Catheters / Lines',n.Indwelling_Catheters)}${ta('otherInvestigations','Other Investigations',n.Other_Investigations,true)}</div><h3 class="system-title">Assessment and plan</h3><div class="form-grid">${ta('assessment','Assessment',n.Assessment,true)}${ta('plan','Plan',n.Plan,true)}</div>`}
...
function activateEditorAutosave_(){const host=document.getElementById('noteEditorForm');if(!host)return;host.addEventListener('input',()=>{state.editorDirty=true;setLocalSaveStatus_('Saving locally…');clearTimeout(state.editorSaveTimer);state.editorSaveTimer=setTimeout(flushLocalDraft_,450)});flushLocalDraft_();}
...
function applyCopy(n){document.querySelectorAll('[data-copy]:checked').forEach(ch=>{const id=ch.dataset.copy,key={hopi:'HOPI',problems:'Problems',cns:'CNS',cvs:'CVS',respiratory:'Respiratory',git:'GIT',renal:'Renal',haematology:'Haematology',infection:'Infection',indwellingCatheters:'Indwelling_Catheters',otherInvestigations:'Other_Investigations',assessment:'Assessment',plan:'Plan'}[id];const el=document.getElementById(id);if(el)el.value=n[key]||''});state.currentNote=state.currentNote||{};state.currentNote.copiedFromNoteId=n.Note_ID;closeModal();flushLocalDraft_();toast('Selected sections copied. Review and update before signing.')}
